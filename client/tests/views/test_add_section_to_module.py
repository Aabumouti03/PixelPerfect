...
class AddSectionToModuleViewTest(TestCase):
    """Test suite for the add_section_to_module view"""
    ...
    def setUpTestData(cls):
        ...
        # Create sample module and section
        cls.module = Module.objects.create(title="Sample Module")
        cls.section = Section.objects.create(title="Sample Section")
        
        # URL for the view
        cls.url = reverse('add_section_to_module', args=[cls.module.id])
    
    def test_admin_can_add_section(self):
        """Test that an admin can successfully add a section to a module"""
        self.client.login(username='adminuser', password='adminpass')
        response = self.client.post(
            self.url,
            data=json.dumps({"section_id": self.section.id}),
        ...
        self.assertIn(self.section, self.module.sections.all())
    ...
    def test_invalid_section_id(self):
        """Test that an invalid section ID returns an error"""
        self.client.login(username='adminuser', password='adminpass')
        response = self.client.post(
            self.url,
            data=json.dumps({"section_id": 9999}),  # Non-existent section
            content_type='application/json'
        )
