...
class ModuleOverviewViewTest(TestCase):
    ...
    def setUp(self):
        """Set up test user, module, exercises, and resources."""
    ...
    def test_module_overview_view(self):
        ...
        self.assertEqual(len(response.context['exercises']), 2)  # 2 exercises expected

        self.assertIn('additional_resources', response.context)
        self.assertEqual(len(response.context['additional_resources']), 2)  # 2 resources expected
    ...
    def test_progress_calculation(self):
        ...
        total_items = 2 + 2  # 2 exercises + 2 resources
        completed_items = 1 + 1  # 1 completed exercise + 1 completed resource
